...
function Category() {
  ...
  const currentCoffee = coffees.slice(indexOfFirstPost, indexOfLastPost)
   // 가격을 정수로 변환
  const strtoint = (dataname) => (dataname).map((item)=>({
    ...item,
  ...
  const paikPrice = strtoint(paikData)
   // 가격정렬(높은순, 낮은순)
  const sortPriceHigh = (priceList) => priceList.sort((a,b)=>b.price - a.price)
  const sortPriceLow = (priceList) => priceList.sort((a,b)=>a.price - b.price)
  // 가격 높은순
  const currentHighStarbucks = sortPriceHigh(starbucksPrice).slice(indexOfFirstPost,indexOfLastPost)
  const currentHighMega = sortPriceHigh(megaPrice).slice(indexOfFirstPost,indexOfLastPost)
  const currentHighEdiya = sortPriceHigh(ediyaPrice).slice(indexOfFirstPost,indexOfLastPost)
  const currentHighHollys = sortPriceHigh(hollysPrice).slice(indexOfFirstPost,indexOfLastPost)
  const currentHighPaik = sortPriceHigh(paikPrice).slice(indexOfFirstPost,indexOfLastPost)
  // 가겨 낮은순
  const currentLowStarbucks = sortPriceLow(starbucksPrice).slice(indexOfFirstPost,indexOfLastPost)
  const currentLowMega = sortPriceLow(megaPrice).slice(indexOfFirstPost,indexOfLastPost)
  const currentLowEdiya = sortPriceLow(ediyaPrice).slice(indexOfFirstPost,indexOfLastPost)
  const currentLowHollys = sortPriceLow(hollysPrice).slice(indexOfFirstPost,indexOfLastPost)
  const currentLowPaik = sortPriceLow(paikPrice).slice(indexOfFirstPost,indexOfLastPost)
  // dropdown 상태관리 함수
  ...
  const starbucksHigh = useCoffeeList(selectedTagCafeId, currentHighStarbucks, activePage, postsPerPage, handleCoffeeDetail);
  const starbucksLow = useCoffeeList(selectedTagCafeId, currentLowStarbucks, activePage, postsPerPage, handleCoffeeDetail);
  const starbucksNew = useCoffeeList(selectedTagCafeId, currentPosts, activePage, postsPerPage, handleCoffeeDetail)
  
  const megaHigh = useCoffeeList(selectedTagCafeId, currentHighMega, activePage, postsPerPage, handleCoffeeDetail);
  const megaLow = useCoffeeList(selectedTagCafeId, currentLowMega, activePage, postsPerPage, handleCoffeeDetail);
  const megaNew = useCoffeeList(selectedTagCafeId, currentMega, activePage, postsPerPage, handleCoffeeDetail)
  
  const ediyaHigh = useCoffeeList(selectedTagCafeId, currentHighEdiya, activePage, postsPerPage, handleCoffeeDetail);
  const ediyaLow = useCoffeeList(selectedTagCafeId, currentLowEdiya, activePage, postsPerPage, handleCoffeeDetail);
  const ediyaNew = useCoffeeList(selectedTagCafeId, currentEdyia, activePage, postsPerPage, handleCoffeeDetail)

  const hollysHigh = useCoffeeList(selectedTagCafeId, currentHighHollys, activePage, postsPerPage, handleCoffeeDetail);
  const hollysLow = useCoffeeList(selectedTagCafeId, currentLowHollys, activePage, postsPerPage, handleCoffeeDetail);
  const hollysNew = useCoffeeList(selectedTagCafeId, currentHollys, activePage, postsPerPage, handleCoffeeDetail)

  const paikHigh = useCoffeeList(selectedTagCafeId, currentHighPaik, activePage, postsPerPage, handleCoffeeDetail);
  const paikLow = useCoffeeList(selectedTagCafeId, currentLowPaik, activePage, postsPerPage, handleCoffeeDetail);
  const paikNew = useCoffeeList(selectedTagCafeId, currentPaik, activePage, postsPerPage, handleCoffeeDetail)
  
  function Dropdown() {
    return (
      <>
       {selectedMenu==='최신순' ? "": <li onClick={() => handleDropdown('최신순')}>최신순</li>}
       {selectedMenu==='평점순' ? "": <li onClick={() => handleDropdown('평점순')}>평점순</li>}
       {selectedMenu==='가격 높은순' ? "": <li onClick={() => handleDropdown('가격 높은순')}>가격 높은순</li>}
       {selectedMenu==='가격 낮은순' ? "": <li onClick={() => handleDropdown('가격 낮은순')}>가격 낮은순</li>}
