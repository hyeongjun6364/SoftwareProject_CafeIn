// src/api/cafe/cafe.ctrl.js// src/api/cafe/cafe.ctrl.js
...
export const getEdiyaMenu = async (ctx) => {
  try {
    ...
    // MongoDB에 저장할 데이터 형식을 Cafe 모델과 일치시킵니다.
    const cafesToSave = jsonData.map((item, index) => ({
      id: index + 1, // 일련번호를 1부터 부여
      name: item.prodName, // prodName을 name 필드에 저장
      image: item.prodImage, // prodImage를 image 필드에 저장
      tag: item.tag || [], // tag 필드를 선택적으로 저장 (없을 경우 빈 배열)
      menu: item.menu || [], // menu 필드를 선택적으로 저장 (없을 경우 빈 배열)
    }));

    // MongoDB에 저장합니다.
    await Cafe.insertMany(cafesToSave);
